...
#define _POLL_WRITE_MULTI_REGISTERS 0x04

#define MAX_QUEUE 8
#define MAX_UNSIG_LONG 0xFFFFFFFF
#define MAX_BUFFER_LENGTH MAX_QUEUE
...
{
    return write_single(_FC_WRITE_MULTIPLE_REGISTERS, addr, value);
}
...
int modbus_write_registers(int addr, int nb, const uint16_t *src)
{
    ...
    req_length = build_req_basis(_FC_WRITE_MULTIPLE_REGISTERS, addr, nb, req);
    byte_count = nb * 2;
...
int modbus_poll( )
{
	...
	switch(poll_status)
	{
		...
		case  _POLL_WRITE_MULTI_REGISTERS:
		{
			//int modbus_write_registers(int addr, int nb, const uint16_t *src)
			unsigned int write_multi_regs[2];

			write_multi_regs[0] = queue_out()&0x0000ffff ;   //去掉32位MSB 
			write_multi_regs[1] = queue_out()&0x0000ffff;
			
			if(write_multi_regs[0] != 0xffff&&write_multi_regs[1] != 0xffff)
				nb = modbus_write_registers(0x09,2,write_multi_regs);
			else{
				memset(write_multi_regs,0,sizeof(write_multi_regs));
			}
			...
			if(nb) 
			 {
				...
				if(DC_Volt_gain != dest[0] || DC_Cur_gain != dest[1])
				{
					...
					if(write_times  < 10)
					{
						...
						queue_in(0x09,DC_Volt_gain+1);
						queue_in(0x0A,DC_Cur_gain+1);
						write_times ++;
				...
				if((write_times>=10)&&((DC_Volt_gain!=10000)||(DC_Cur_gain!=10400)))
				{
...
unsigned long queue_out()
{
	...
	if(queue_ptn != 0)
	{
		...
		{	
			...
			write_out_buffer[i-1] = 0; // 数组向前移动一位，去掉最后一位数据使其归零;
...
void get_queue_data_to_write()
{
	...
	{
		write_reg_addr	=	ret_val >> 16;          //MSB shift 16 bits 到发送缓存区
		write_reg_value	= ret_val & 0x0000ffff; //LSB 填充好
...
void optimize_buffer_sort()
{
	if(queue_ptn>6)
	{
		shaker_sort(write_out_buffer,queue_ptn);
	}
}
...
void shaker_sort(unsigned long *element,const unsigned char length)
{
	...
	if(length <= MAX_BUFFER_LENGTH )
		while(!guard)   //判断是否发生交换，没有交换认为sort ok；
		{
			if(i==0)    //从头往后 sort ；
			{
				while(i!=MAX_BUFFER_LENGTH)
				{
					...
					{
						exchange_val(&element[i],&element [i+1]);
						guard = true;
					}
				...
				i =MAX_BUFFER_LENGTH-1;   //减去多余的 i++;
				
				if(guard == true)   //判断是否发生交换 ；
					guard = false;
				else
					guard = true;
